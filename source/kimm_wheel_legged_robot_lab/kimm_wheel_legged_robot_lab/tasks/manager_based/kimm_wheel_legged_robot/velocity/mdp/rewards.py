...
def debug(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    """
    asset: RigidObject = env.scene[asset_cfg.name]
    com_pos_z = asset.data.body_com_pose_w[:, :, 2] # (num_envs, num_bodies)
    
    masses = asset.root_physx_view.get_masses() # (num_envs, num_bodies)
    masses = masses.to(env.device)
    # com_pos_z = com_pos_z * masses
    # com_pos_z = torch.sum(com_pos_z, dim=1) / torch.sum(masses)
    com = torch.sum(com_pos_z * masses, dim=1) / torch.sum(masses, dim=1)
    print(f"com: {com[0]}")

    body_id, body_name = asset.find_bodies("pelvis")
    print(f"body_id: {body_id}")
    base_height = asset.data.body_pos_w[:, body_id, 2].squeeze(-1)
    print(f"pelvis height: {base_height[0]}") # 0.4683942198753357
